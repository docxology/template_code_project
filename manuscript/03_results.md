...
: Gradient descent outcomes per configured step size: state at termination, iteration count capped by $N_{\max} = {{CONFIG_MAX_ITERATIONS}}$, the `converged` flag from `gradient_descent()` using $\|\nabla f\| < {{CONFIG_TOLERANCE}}$, and its structured termination reason. A `non_finite` row stops before an overflowing update is serialized and retains the last finite iterate. {#tbl:opt_results}
...
[@fig:benchmark] shows deterministic computational work and convergence iterations for `gradient_descent()` on identity-Hessian quadratics of dimension $d \in \{{{CONFIG_BENCHMARK_DIMS}}\}$.
